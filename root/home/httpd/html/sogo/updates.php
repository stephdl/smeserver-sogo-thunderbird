...
if ($appversion === '10') { /* Thunderbird 10 */
	$plugins_globpath = "thunderbird10/" . $plugins_filename[$pluginid];
	$plugins = array(
		"minVersion" => "10.0",
		"maxVersion" => "10.1.*"
	);
} else if ($appversion === '17') { /* Thunderbird 17 */
    if($pluginid === '{e2fda1a4-762b-4020-b5ad-a41df1933103}') {
        # Lightning without '-inverse' suffix:        
        $plugins_globpath = "thunderbird17/" . $platform . "/lightning-*.xpi";
    } else {
        $plugins_globpath = "thunderbird17/" . $plugins_filename[$pluginid];
    } 
	$plugins = array(
		"minVersion" => "17.0",
		"maxVersion" => "17.1.*"
	);
} else if ($appversion === '24') { /* Thunderbird 24 */
    if($pluginid === '{e2fda1a4-762b-4020-b5ad-a41df1933103}') {
        # Lightning without '-inverse' suffix:        
        $plugins_globpath = "thunderbird24/" . $platform . "/lightning-*.xpi";
    } else {
        $plugins_globpath = "thunderbird24/" . $plugins_filename[$pluginid];
    } 
	$plugins = array(
		"minVersion" => "24.1.1",
		"maxVersion" => "24.*"
